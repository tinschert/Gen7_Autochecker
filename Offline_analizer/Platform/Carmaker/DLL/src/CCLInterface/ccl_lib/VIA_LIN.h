...
public:
  ...
  VIASTDDECL CreateSchedulerModeChangeRequest2(VIARequestHandle* handle,      // [IN] Created request handle
    ...
    VIAChannel channel) = 0;              // [IN] LIN channel or -1 (for all channels)
#endif
#if defined ( VIALINMinorVersion) && ( VIALINMinorVersion >= 10)
  // >>> https://jira.vg.vector.int/browse/CANOE-146201
  VIASTDDECL LINIsChannelMasterMode(VIAChannel channel, uint8& isMaster) = 0;
  VIASTDDECL LINSetChannelMasterMode(VIAChannel channel, uint8 isMaster) = 0;
  VIASTDDECL LINIsCurrentMasterNode(VIAChannel channel, uint8& isMaster) = 0;
  VIASTDDECL LINGetCurrentMasterNode(VIAChannel channel, uint32& clientIndex) = 0;
  VIASTDDECL LINSetCurrentMasterNode(VIAChannel channel, uint32& previousMasterClientIndex) = 0;
  VIASTDDECL LINSetCurrentMasterNodeWithClientIndex(VIAChannel channel, uint32 clientIndex, uint32& previousMasterClientIndex) = 0;
  VIASTDDECL LINSetInterruptingHeaderTransmits(VIAChannel channel, uint32 onoff) = 0;
  // <<<
#endif
};
