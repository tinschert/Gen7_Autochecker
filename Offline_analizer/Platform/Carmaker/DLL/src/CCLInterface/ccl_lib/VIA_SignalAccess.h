...
2019-06-05   2.15 Wep     MemorySegment Object: added support for SysVars (CANoe 12.0 SP3)
-----------------------------------------------------------------------------*/

#define VIASignalAccessMajorVersion 2
#define VIASignalAccessMinorVersion 15
...
public:
  ...
  //             1: value is in range
  // Supported by CAPL-on-Board: No
  VIASTDDECL CheckSignalInRange(double aLowLimit, double aHighLimit, int32* aRet) = 0;
  ...
  // kVIA_SigInteger:   For int32 or int64
...
#if (VIASignalAccessMajorVersion >= 3) || (VIASignalAccessMajorVersion == 2 && VIASignalAccessMinorVersion >= 15)
  // Sets the client handle for system variables. Must be done before adding
  // system variables.
  VIASTDDECL SetSysVarClientHandle(VIASysVarClientHandle* clientHandle) = 0;

  // Adds a CANoe system variable to the memory segment at the given offset.
  // SetSysVarClientHandle must have been called before.
  // Currently only working for variables of type integer and float.
  // Variables of type 'integer' are stored as a 32 bit signed integer value 
  //  (4 Bytes required)
  // Variables of type 'float' are stored as 64 bit floating point values 
  //  (8 Bytes required). 
  VIASTDDECL AddSysVar(VIASystemVariable* sysVar, uint32 offset, uint32 maxsize = 0) = 0;
#endif
};
...
class VIASignalAccessApi_V1
{
  public:
    ...
    // Obsolete (version 1.1)
    VIASTDDECL CheckSignalInRange(const char * aSignal, const char * aMsg, const char * aDB, double aLowLimit, double aHighLimit, int32* aRet) = 0;
    // Obsolete (version 1.1)
    VIASTDDECL GetSignalGM(const char * aSignal, const char * aMsg, const char * aDB, uint32 aSourceId, double* aValue) = 0;
    // Obsolete (version 1.1)
    VIASTDDECL GetSignalTimeGM(const char * aSignal, const char * aMsg, const char * aDB, uint32 aSourceId, VIATime* aTime) = 0;
    // Obsolete (version 1.1)
    VIASTDDECL CheckSignalInRangeGM(const char * aSignal, const char * aMsg, const char * aDB, uint32 aSourceId, double aLowLimit, double aHighLimit, int32* aRet) = 0;
};
