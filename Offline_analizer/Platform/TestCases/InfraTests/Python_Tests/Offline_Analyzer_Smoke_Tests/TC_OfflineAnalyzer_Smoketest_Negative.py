...
def TC_OfflineAnalyzer_Smoketest_Negative(input_log):
    ...
    # Analysis
    trace.check_signal_update(data, Condition.NOT_EQUALS, 3.84)
    ...
    # Analysis
    trace.check_signal_update(data, Condition.EQUALS,20)
    HTML_Logger.ReportTestStepEnd()
    ...
    # Analysis
    trace.check_signal_update(data, Condition.NOT_EQUALS,"Defender")
    HTML_Logger.ReportTestStepEnd()
    ...
    data_input = trace.get_signal_value(channels["ADAS_1C0_AliveCounter"])  # 0,0 - all range, x,0 - from x:end, 0,x - beginning:x , x,x - defined range
    data_to_check = trace.get_signal_value(channels["Vehicle_Variant"])  # 0,0 - all range, x,0 - from x:end, 0,x - beginning:x , x,x - defined range
    ...
    data = trace.get_signal_value(channels["ADAS_1C0_AliveCounter"])  # 0,0 - all range, x,0 - from x:end, 0,x - beginning:x , x,x - defined range
    # Analysis
    trace.check_signal_update(data, Condition.WITHIN_RANGE, signal_range=[30,50])
    HTML_Logger.ReportTestStepEnd()
    ...
    data = trace.get_signal_value(channels["ADAS_1C0_AliveCounter"])  # 0,0 - all range, x,0 - from x:end, 0,x - beginning:x , x,x - defined range
    # Analysis
    trace.check_incrementing(data, 16)
    HTML_Logger.ReportTestStepEnd()
    ...
    data = trace.get_signal_value(channels["ADAS_1C0_AliveCounter"])  # 0,0 - all range, x,0 - from x:end, 0,x - beginning:x , x,x - defined range
    # Analysis
    trace.check_pattern_with_delta(data, 0, 1, 15)
    HTML_Logger.ReportTestStepEnd()
...
if __name__ == "__main__":
    ...
    if os.path.isdir(LOG):
        ...
        for log in logs:
            TC_OfflineAnalyzer_Smoketest_Negative(log)
    else:
        TC_OfflineAnalyzer_Smoketest_Negative(LOG)
