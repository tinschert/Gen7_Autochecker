...
#INPUT_LOG = r"C:\Users\pep3sf4\Desktop\Test_mf4\Vehicle_Model_Test_1_Config2_T001.mf4"

def TC_OfflineAnalyzer_Bytearray_Smoketest(input_log):

    ################# Convert input MF4 file to ASC file format ##################################################
    LOG = asc_parser.convert_mf4_to_asc(input_log)
    ...
    asc_data = asc_parser.parse_asc(LOG, RADAR_ID.RFL, objects_to_analyze = 3)
    if asc_data:
        signal_data = asc_data["RFL_Object_SpiHdr_SensorOriginPointX"]
        trace.check_signal_update(signal_data, Condition.EQUALS, 3.32)  # Returns bool(True/False) and pd.Dataframe
    HTML_Logger.ReportTestStepEnd()

    HTML_Logger.ReportTestStepStart()
    HTML_Logger.ReportWhiteMessage(f"--------------- Test Step 2 --------------------")
    asc_data = asc_parser.parse_asc(LOG, RADAR_ID.RFR, objects_to_analyze = 3)
    if asc_data:
        signal_data = asc_data["RFR_Object_SpiHdr_SensorOriginPointX"]
        trace.check_signal_update(signal_data, Condition.EQUALS, 3.32)  # Returns bool(True/False) and pd.Dataframe
    HTML_Logger.ReportTestStepEnd()
    ################################################################################################################
...
if __name__ == "__main__":
    ...
    if os.path.isdir(INPUT_LOG):
        ...
        for log in logs:
            TC_OfflineAnalyzer_Bytearray_Smoketest(log)
    else:
        TC_OfflineAnalyzer_Bytearray_Smoketest(INPUT_LOG)
