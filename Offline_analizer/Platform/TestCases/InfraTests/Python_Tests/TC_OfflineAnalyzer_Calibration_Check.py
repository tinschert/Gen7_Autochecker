...
LOG = r"C:\Users\tic7lr\Documents\Temp\Calibration_test" # Single log example
...
def TC_ACC_Test(input_log):
    ...
    channels = output.get_channels(["_g_Common_VAG_1R1V_Cal_MalRunnable_MalRunnable_m_dynCalibResponsePort_out_local.TChangeableMemPool._._._m_arrayPool._1_._elem._m_status",
                                    "_g_Common_VAG_1R1V_Cal_MalRunnable_MalRunnable_m_dynCalibResponsePort_out_local.TChangeableMemPool._._._m_arrayPool._1_._elem._m_progress", 
                                    "_g_Common_VAG_1R1V_Cal_MalRunnable_MalRunnable.PerRunnable._._m_controller._m_state._azOutOfSpec._m_stablePosition._m_value",
    ...
    Status_data = trace.get_signal_value(channels["_g_Common_VAG_1R1V_Cal_MalRunnable_MalRunnable_m_dynCalibResponsePort_out_local.TChangeableMemPool._._._m_arrayPool._1_._elem._m_status"], 0, 0) # 0,0 - all range, x,0 - from x:end, 0,x - beginning:x , x,x - defined range
    STC_PASSED_Timestamps = find_stc_passed_timestamps(Status_data)

    azimuth_data = trace.get_signal_value(channels["_g_Common_VAG_1R1V_Cal_MalRunnable_MalRunnable.PerRunnable._._m_controller._m_state._azOutOfSpec._m_stablePosition._m_value"], 0, 0)
    azimuth_values = get_signal_values_at_timestamps(azimuth_data, STC_PASSED_Timestamps)
    
    elevation_data = trace.get_signal_value(channels["_g_Common_VAG_1R1V_Cal_MalRunnable_MalRunnable.PerRunnable._._m_controller._m_state._elOutOfSpec._m_stablePosition._m_value"], 0, 0)
    elevation_values = get_signal_values_at_timestamps(elevation_data, STC_PASSED_Timestamps)

    plot_points(azimuth_values, elevation_values, output_file='plot.png')
    
    # Analysis
    trace.check_signal_update(data, Condition.EQUALS, "STC_PASSED") # Returns bool(True/False) and pd.Dataframe
