...
def TC_CarMaker_Test_Ego_Long_Target_Long():
    
    capl.Testcase_Start(__file__, "CANoePy Testcase", filename=HTML_Logger.generate_report_name()) #create the HTML report
    HTML_Logger.TestReportHeader("Example : CANoePy v1.1 CarMaker_Test_Ego_Long_Target_Long Test with ADAS HIL 2.xx")
    HTML_Logger.TestReportHeader("Tester : Ventsislav Negentsov")
    HTML_Logger.TestReportHeader("TestCaseID : TestCase_CarMaker_Test_2")
    ...
    capl.TestWaitForTimeout(1000)
    capl.SetSignal("Customer_specific", "cm_scenario", "AES_EgoLong_TargetLong_Car__AEB_CCRs")  # fill the scenario name
    capl.TestWaitForTimeout(1000)
    capl.CheckSignal("Customer_specific", "cm_scenario","AES_EgoLong_TargetLong_Car__AEB_CCRs")  # check if scenario field is populated
    ...
    capl.WaitForSignalInRange("CarMaker", "RB/TrafficObj/object0/dtx", 20, 25, 60)
...
if __name__ == "__main__":
    TC_CarMaker_Test_Ego_Long_Target_Long()
