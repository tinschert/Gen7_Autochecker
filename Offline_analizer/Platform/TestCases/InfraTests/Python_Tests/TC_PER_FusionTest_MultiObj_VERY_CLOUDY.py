...
def TC_PER_FusionTest_MultiObj_VERY_CLOUDY():
    ...
    HTML_Logger.TestReportHeader("TestCaseID : TC_PER_FusionTest_MultiObj_VERY_CLOUDY")
    ...
    capl.TestWaitForTimeout(1000)
    capl.SetSignal("Customer_specific", "cm_scenario", r"Infra_scenarios\\PER_tests\\Fusion_MultiObj_VERY_CLOUDY")  # fill the scenario name
    capl.TestWaitForTimeout(1000)
    ...
    tf.Set_Ego_Vehicle_Velocity(60.0)  # set 40kph
    tf.Set_Drv_gear(3)  # set Drive gear
    capl.SetSignal_Array("", "Classe_Obj_Sim/obj_ctrl.obj_target_v_long", [15, 35, 55, 15, 55, 55, 55, 55, 55, 55, 55])   #set target object to 40kph
    ...
    capl.TestWaitForTimeout(13000)
    capl.SetSignal("", "hil_ctrl/Ego_lane_offset", 3.5)  # move left lane
    capl.TestWaitForTimeout(25000)
    #capl.SetSignal("","hil_drv/high_beam_is_on",0)   #high beam OFF
    #capl.SetSignal("","hil_drv/low_beam_is_on", 0)   #low beam OFF
    #capl.SetSignal("","hil_drv/indicator_light", 0)  #no turn indicator lights
...
if __name__ == "__main__":
    TC_PER_FusionTest_MultiObj_VERY_CLOUDY()
