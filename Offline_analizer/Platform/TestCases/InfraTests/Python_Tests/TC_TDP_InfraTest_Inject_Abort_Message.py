...
def TC_TDP_InfraTest_Inject_Abort_Message():
    ...
    #trigger a TDP fault : inject an abort message during scenario run
    HTML_Logger.ReportWhiteMessage("\n\n----------------------------------------------------------------------------------------------------------------------------------------------------------------")
    HTML_Logger.ReportBlueMessage("!!! Inject HIL abort message during a scenario run !!!\n")
    capl.SetSignal("","hil_ctrl/abort_message","Python TDP Error #1")
    ...
    for el in f1:
        if el.find(r"Abort Message : Python TDP Error") >= 0:
...
if __name__ == "__main__":
    TC_TDP_InfraTest_Inject_Abort_Message()
