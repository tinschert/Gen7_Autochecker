...
write_cycle_time = 0.08 #80ms

def TC_API_Perfo_Write_Multiple_Bus_Signals_Cyclic(test_duration = 10):   #default test duration 10 seconds
    
    capl.Testcase_Start(__file__, "CANoePy Testcase", filename=HTML_Logger.generate_report_name()) #create the HTML report
    HTML_Logger.TestReportHeader("Example : CANoePy v1.1 API_Perfo_Write_Multiple_Bus_Signals_Cyclic with ADAS HIL 2.xx")
    HTML_Logger.TestReportHeader("Tester : Ventsislav Negentsov")
    HTML_Logger.TestReportHeader("TestCaseID : TestCase_API_Perfo_Write_Multiple_Bus_Signals_Cyclic")
    ...
    counter = 0
    counter2 = 0
    num_loops_for_required_test_duration = test_duration/write_cycle_time/4/2
    #print(num_loops_for_required_test_duration)
    while (counter2<=num_loops_for_required_test_duration):
        while(counter<=1):
            capl.SetCheckSignal("", "CAN_ECAN/OneDriving_CR_Public_CANFD/MGW/ESP_10/ESP_QBit_Wegimpuls_HL", counter, write_cycle_time)
            capl.SetCheckSignal("", "CAN_ECAN/OneDriving_CR_Public_CANFD/MGW/ESP_10/ESP_QBit_Wegimpuls_HR", counter, write_cycle_time)
            capl.SetCheckSignal("", "CAN_ECAN/OneDriving_CR_Public_CANFD/MGW/ESP_10/ESP_QBit_Wegimpuls_VL", counter, write_cycle_time)
            capl.SetCheckSignal("", "CAN_ECAN/OneDriving_CR_Public_CANFD/MGW/ESP_10/ESP_QBit_Wegimpuls_VR", counter, write_cycle_time)
            counter = counter + 1
        counter = 0
        counter2=counter2+1
...
if __name__ == "__main__":
    TC_API_Perfo_Write_Multiple_Bus_Signals_Cyclic()
    capl.Disconnect()
    capl.Dispose()
