...
def write_to_access_database(
    ...
    negativeTheta_comment, filterPhiSpread_check, negativeTheta_count, doppler_critical_count, 
    doppler_check, numLargerSOS_count,stable_el_average,stable_az_average,stable_comment,full_brake_check):
    """
        ...
        if exists:
            ...
            cursor.execute(
                ...
                (
                    ...
                    numLargerSOS_count,stable_el_average,stable_az_average,stable_comment,full_brake_check,measurement_ID
                )
            )
        else:
            # Insert a new row
            cursor.execute(
                ...
                INSERT INTO measurements (
                    ...
                    ,stable_el_average,stable_az_average,stable_comment,full_brake_check
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?)
                """,
                (
                    ...
                    negativeTheta, Check_Doppler_EME_Phispread, numLargerSOS_intervalls,
                    negativeTheta_comment, filterPhiSpread_check, negativeTheta_count, doppler_critical_count, doppler_check,
                    numLargerSOS_count, stable_el_average, stable_az_average, stable_comment,full_brake_check
                )
            )
        conn.commit()
...
def evaluate_full_brake(full_brake_status_data):
    """
    Checks if 'Signal Value' ever becomes 'Active' in the DataFrame.
    If always 'Inactive', returns 'pass'.
    If 'Active' is found, returns 'fail, activation (start_time - end_time)' for each interval.
    """
    df = full_brake_status_data
    active_mask = df["Signal Value"] == "Active"
    ...
    if len(intervals) == 1:
        ...
        return f"fail, activation ({start} - {end})"
    else:
        return [f"fail, activation ({start} - {end})" for start, end in intervals]
...
def TC_ACC_Test(input_log):
    ...
    channels = output.get_channels(["_DspCopyDataPacket._CopyDspMnpDataPacket._vxvRef",
                                    ...
                                    "_g_Common_VAG_OdsRadarServices_Mal_PerMalRunnable_PerMalRunnable.PerRunnable._._m_controller._m_input._elDSPMAL._m_percentNegativeTheta",
                                    "_g_Common_VAG_OdsRadarServices_Mal_PerMalRunnable_PerMalRunnable.PerRunnable._._m_controller._m_state._elCompensation._m_filterPhiSpread",
                                    "_g_Common_VAG_OdsRadarServices_Mal_PerMalRunnable_PerMalRunnable.PerRunnable._._m_controller._m_state._elEMEIndicator._m_estimation._m_val._m_value",
                                    ...
                                    "_g_Common_VAG_OdsRadarServices_Mal_PerMalRunnable_PerMalRunnable.PerRunnable._._m_controller._m_state._elIndicator._m_estimation._m_val._m_value",
                                    "_g_Common_VAG_OdsRadarServices_Mal_PerMalRunnable_PerMalRunnable.PerRunnable._._m_controller._m_state._elOutOfSpec._m_stablePosition._m_value",
                                    "_g_Common_VAG_OdsRadarServices_Mal_PerMalRunnable_PerMalRunnable.PerRunnable._._m_controller._m_state._azOutOfSpec._m_stablePosition._m_value",
                                    "_g_Common_VAG_Function_SafetyFunctions_SfRunnableMainProc_SfRunnableMainProc.RunnableMainProcSfBase._._m_stateMachines._m_aebFullBrakeStateMachine.PssStateMachine._._m_currentState"]) # Provide channels of interest as a string list. Functions will return mdf channel objects
    ...
    filterPhiSpread = trace.get_signal_value(channels["_g_Common_VAG_OdsRadarServices_Mal_PerMalRunnable_PerMalRunnable.PerRunnable._._m_controller._m_state._elCompensation._m_filterPhiSpread"], 0, 0)

    percentNegativeTheta_Data = trace.get_signal_value(channels["_g_Common_VAG_OdsRadarServices_Mal_PerMalRunnable_PerMalRunnable.PerRunnable._._m_controller._m_input._elDSPMAL._m_percentNegativeTheta"], 0, 0)
    ...
    Doupler_elIndicator_data = trace.get_signal_value(channels["_g_Common_VAG_OdsRadarServices_Mal_PerMalRunnable_PerMalRunnable.PerRunnable._._m_controller._m_state._elIndicator._m_estimation._m_val._m_value"], 0, 0)
    EME_Indicator_data = trace.get_signal_value(channels["_g_Common_VAG_OdsRadarServices_Mal_PerMalRunnable_PerMalRunnable.PerRunnable._._m_controller._m_state._elEMEIndicator._m_estimation._m_val._m_value"], 0, 0)
    Stable_Position_data = trace.get_signal_value(channels["_g_Common_VAG_OdsRadarServices_Mal_PerMalRunnable_PerMalRunnable.PerRunnable._._m_controller._m_state._elOutOfSpec._m_stablePosition._m_value"], 0, 0)
    Stable_Position_az_data = trace.get_signal_value(channels["_g_Common_VAG_OdsRadarServices_Mal_PerMalRunnable_PerMalRunnable.PerRunnable._._m_controller._m_state._azOutOfSpec._m_stablePosition._m_value"], 0, 0)
    ...
    stable_el_average, stable_el_minimum, stable_el_maximum, stable_el_std_dev = get_signal_statistics(Stable_Position_data)
    stable_az_average, stable_az_minimum, stable_az_maximum, stable_az_std_dev = get_signal_statistics(Stable_Position_az_data)

    if stable_el_std_dev == 0 and stable_az_std_dev == 0:
        stable_comment = "No change in stable position"
    elif stable_el_std_dev != 0:
        stable_comment = "Change in elevation"
    elif stable_az_std_dev != 0:
        stable_comment = "Change in azimuth"

    full_brake_status_data = trace.get_signal_value(channels["_g_Common_VAG_Function_SafetyFunctions_SfRunnableMainProc_SfRunnableMainProc.RunnableMainProcSfBase._._m_stateMachines._m_aebFullBrakeStateMachine.PssStateMachine._._m_currentState"], 0, 0)
    full_brake_check = evaluate_full_brake(full_brake_status_data)
    ...
    write_to_access_database(measurement_ID, measurement_type, timestamp_start, timestamp_end ,measurement_duration,
                             ...
                             negativeTheta_count, doppler_critical_count,doppler_check,numLargerSOS_count,
                             stable_el_average,stable_az_average,stable_comment,full_brake_check)  # Write the measurement ID and timestamps to the Access database
    print("scirpt done!")
