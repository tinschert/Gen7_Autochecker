...
#include "CVehicleTwoWheeler.h"

/*------------------------*/
/* constructor/destructor */
/*------------------------*/
CVehicleTwoWheeler::CVehicleTwoWheeler( CInt numberOfRadars, CInt numberOfVideos, CInt numberOfLidars ) : CModule<NumberOfStates>(),
...
{
    addMessageOutput( o_numberOfRadars, numberOfRadars, CVehicleTwoWheelerDoc::o_numberOfRadars );
    addMessageOutput( o_numberOfVideos, numberOfVideos, CVehicleTwoWheelerDoc::o_numberOfVideos );
    addMessageOutput( o_numberOfLidars, numberOfLidars, CVehicleTwoWheelerDoc::o_numberOfLidars );

    o_zero.setInit( 0.0 );
    addMessageOutput( o_zero, 0.0 );
    o_zeroVector.setInit( CFloatVectorXYZ( 0.0, 0.0, 0.0 ) );
    addMessageOutput( o_zeroVector, CFloatVectorXYZ( 0.0, 0.0, 0.0 ) );

    /* Initialization messages */
    addMessageParameter( p_casterAngle, 30 * M_PI / 180., CVehicleTwoWheelerDoc::p_casterAngle );  // 30 degree caster angle
    addMessageParameter( p_lateralOffset, 0.0, CVehicleTwoWheelerDoc::p_lateralOffset );
    addMessageParameter( p_targetLateralOffset, 0.0, CVehicleTwoWheelerDoc::p_targetLateralOffset );
    addMessageParameter( p_lateralVelocity, 0.0, CVehicleTwoWheelerDoc::p_lateralVelocity );
    addMessageParameter( p_road, 0LL, CVehicleTwoWheelerDoc::p_road );
    addMessageParameter( p_lane, 0LL, CVehicleTwoWheelerDoc::p_lane );
    ...
    addMessageOutput( o_xyzWorld, CFloatVectorXYZ( 0.0, 0.0, 0.0 ), CVehicleTwoWheelerDoc::o_xyzWorld );
    addMessageOutput( o_latitude, 0.0 );
    addMessageOutput( o_longitude, 0.0 );
    addMessageOutput( o_s, 0.0, CVehicleTwoWheelerDoc::o_s );

    // not changeabel by scene => no setMessage description
    addMessageOutput( o_angleRollPitchYawSurface, CFloatVectorXYZ( 0.0, 0.0, 0.0 ) );

    addMessageOutput( o_angleRollPitchYawChassis, CFloatVectorXYZ( 0.0, 0.0, 0.0 ), CVehicleTwoWheelerDoc::o_angleRollPitchYawChassis );
    addMessageOutput( o_gammaRelativeCourse, 0.0, CVehicleTwoWheelerDoc::o_gammaRelativeCourse );
    addMessageOutput( o_beta, 0.0, CVehicleTwoWheelerDoc::o_beta );
    addMessageOutput( o_velocity, 0.0, CVehicleTwoWheelerDoc::o_velocity );
    addMessageOutput( o_vChassis, CFloatVectorXYZ( 0.0, 0.0, 0.0 ), CVehicleTwoWheelerDoc::o_vChassis );
    addMessageOutput( o_vWorld, CFloatVectorXYZ( 0.0, 0.0, 0.0 ), CVehicleTwoWheelerDoc::o_vWorld );
    addMessageOutput( o_yawRate, 0.0, CVehicleTwoWheelerDoc::o_yawRate );
    addMessageOutput( o_road, p_road, CVehicleTwoWheelerDoc::o_road );
    addMessageOutput( o_lane, p_lane, CVehicleTwoWheelerDoc::o_lane );
    addMessageOutput( o_lateralOffset, p_lateralOffset, CVehicleTwoWheelerDoc::o_lateralOffset );
    addMessageOutput( o_sCourse, 0.0, CVehicleTwoWheelerDoc::o_sCourse );
    addMessageOutput( o_searchIndexLane, 0LL, CVehicleTwoWheelerDoc::o_searchIndexLane );
    addMessageOutput( o_distanceToCourse, 0.0, CVehicleTwoWheelerDoc::o_distanceToCourse );
...
CVehicleTwoWheeler::~CVehicleTwoWheeler()
{}

/*------------------*/
/* public methods  */
/*------------------*/
void CVehicleTwoWheeler::init( CRoadNetwork& f_roadNetwork_r,
                               ...
                               IMessage<CBool>& f_staticSimulation )
{
    ...
    leaning.init( steeringSystem.o_angleWheelFront,
                  chassis.o_vChassis,
                  o_angleRollPitchYawChassis,
                  chassis.p_xWheelFront,
                  chassis.p_xWheelRear,
                  chassis.kineticsFront.o_alpha, //sideslip angles
                  chassis.kineticsRear.o_alpha,
                  p_casterAngle );

    steeringSystem.init( dashboard.o_angleSteeringWheelTarget );

    driveTrain.init( dashboard.o_accelerator,
                     chassis.o_nWheelFront,
                     chassis.o_nWheelRear,
                     dashboard.p_clutch,
                     dashboard.p_gearStick,
                     dashboard.o_brakepedal,
                     dashboard.p_parkingBrake,
                     chassis.staticTwoWheeler.o_FChassis,
                     chassis.staticTwoWheeler.o_FvRChassis,
                     chassis.p_m,
                     chassis.wheelFront.p_rWheel,
                     chassis.wheelRear.p_rWheel,
                     o_angleRollPitchYawSurface );

    chassis.init( steeringSystem.o_angleWheelFront,
                  driveTrain.o_MWheelFront,
                  driveTrain.o_RvMWheelFront,
                  driveTrain.o_MWheelRear,
                  driveTrain.o_RvMWheelRear,
                  o_angleRollPitchYawSurface,
                  leaning.o_rollAngle,
                  o_zeroVector, // articulation force
                  i_staticSimulation );
...
/* private methods */
/*------------------*/
void CVehicleTwoWheeler::calcPre( CFloat f_dT, CFloat f_time )
{
    /* calculate vehicle components */
    dashboard.process( f_dT, f_time );
    driver.process( f_dT, f_time );
    leaning.process( f_dT, f_time );
    ...
    {
        for( CInt t_indexLane = 0; t_indexLane < m_roadNetwork_p->o_NumberOfRoads.get(); t_indexLane++ )
        {
...
/* ddt */
CFloatVector& CVehicleTwoWheeler::ddt( CFloatVector& state )
{
...
void CVehicleTwoWheeler::calcPost( CFloat f_dT, CFloat f_time )
{
    ...
    if( l_laneConnected )
    {
        ...
        SCoursePositionInfo m_newLanePositionInfo_s = m_roadNetwork_p->roads[p_road].lanes[p_lane].getCoursePositionInfo( 0, p_lateralOffset );
    ...
    else if( o_sCourse > m_roadNetwork_p->roads[p_road].o_lengthOfCourse.get() - o_velocity * f_dT )
    {
        l_jumpDetected = true;
    }
    ...
    if( l_jumpDetected )
    {
        SCoursePositionInfo m_positionInfo_s = m_roadNetwork_p->roads[p_road].lanes[p_lane].getCoursePositionInfo( 0, p_lateralOffset );
