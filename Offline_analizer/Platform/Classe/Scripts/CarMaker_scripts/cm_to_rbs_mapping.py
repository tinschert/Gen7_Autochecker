...
column_names = ["Name", "Message", "Value Type", "Initial Value", "Factor", "Offset", "Minimum", "Maximum", "Unit",
                "texttable", "texttable values", "max_value", "variant", "sender", "direction", "texttable mapping",
                "conversion", "array name", "data_type"]
...
def generate_dataframe(wb, sheet_name: str):
    """
    Function to generate main dataframe from excel sheet

    Args:
      wb (openpyxl object): Openpyxl object which will be converted to dataframe
    ...
    try:
        ...
        return False
    [col_name, col_namespace, col_value_type, col_initial_value, col_factor, col_offset, col_minimum, col_maximum,
     col_unit, col_texttable, col_texttable_values, col_max_value, col_variant, col_sender, col_direction,
     col_texttable_mapping, col_conversion, col_array_name, col_data_type] = col_index
...
def generate_onsysvarenum() -> None:
    """
    Function to generate only enumeration type of variables
    ...
    for i in range(num_of_rows_enum):
        if (enum_df[col_sender].values[i] == "carmaker"):
            ...
            if (i == num_of_rows_enum - 1) or (enum_df[col_sender].values[i + 1] == "carmaker") or (
                    enum_df[col_name].values[i + 1] == "na"):
                continue
            ...
            if (sheet == "cm_in"):
                ...
                if enum_df[col_variant].values[i] == "multi":
                    ...
                    idx = 1
                    while "classe" in enum_df[col_sender].values[i + idx] or "Rbs" in enum_df[col_sender].values[
                        i + idx] or "XCP" in enum_df[col_sender].values[i + idx]:
                        namespace_b.append(enum_df[col_namespace].values[i + idx])
                        signal_name_b.append(enum_df[col_name].values[i + idx])
                        init_val = enum_df[col_initial_value].values[i + idx]
                        if init_val != "":
            ...
            if enum_df[col_variant].values[i] != "multi":
                ...
                for k in range(enum_length):
                    ...
                    for variant in range(0, len(combined_data_common)):
                        ...
                        if "XCP" in sender:
                            node.append(
                                f'      @{sender}::{combined_data_common[variant][0][:-1]}::{combined_data_common[variant][1]} = {val};')
                        elif (sheet == "fdx_in" and sheet != "fdx_out" and sender != "classe"):
                            node.append(
                                f'      @{combined_data_common[variant][0]}::{combined_data_common[variant][1]}_Rv = {val};')
                        else:
                            node.append(
                                f'      @{combined_data_common[variant][0]}::{combined_data_common[variant][1]} = {val};')
                    node.append('      break;')
                ...
                for variant in range(0, len(combined_data_common)):
                    if "XCP" in sender:
                        node.append(
                            f'      @{sender}::{combined_data_common[variant][0][:-1]}::{combined_data_common[variant][1]} = {combined_data_common[variant][2]};')
                    elif (sheet == "fdx_in" and sheet != "fdx_out" and sender != "classe"):
                        node.append(
                            f'      @{combined_data_common[variant][0]}::{combined_data_common[variant][1]}_Rv = {combined_data_common[variant][2]};')
                    else:
                        node.append(
                            f'      @{combined_data_common[variant][0]}::{combined_data_common[variant][1]} = {combined_data_common[variant][2]};')
                node.append('      break;')
                node.append('  }')
                message_count += 1
            else:
                ...
                for variant in range(0, len(combined_data)):
                    if "XCP" in sender:
                        node.append(
                            f'      @{sender}::{combined_data[variant][0][:-1]}::{combined_data[variant][1]} = {combined_data[variant][2]};')
                    elif (sheet == "fdx_in" and sheet != "fdx_out" and sender != "classe"):
                        node.append(
                            f'      @{combined_data[variant][0]}::{combined_data[variant][1]}_Rv = {combined_data[variant][2]};')
                    else:
                        node.append(
                            f'      @{combined_data[variant][0]}::{combined_data[variant][1]} = {combined_data[variant][2]};')
                node.append('      break;')
                node.append('  }')
                message_count += 1
    node.append('}')
...
def generate_onsysvardouble():
    ...
    for i in range(num_of_rows_double):
        if sysvardouble_df[col_sender].values[i] == "carmaker":
            ...
            add_sysvar_df.columns = [i for i in range(len(col_index))]
            sysvar_df = pd.concat([sysvar_df, add_sysvar_df])
            if (i == num_of_rows_double - 1) or (sysvardouble_df[col_sender].values[i + 1] == "carmaker") or (
                    sysvardouble_df[col_name].values[i + 1] == "na"):
                continue
            if sheet == "cm_in":
            ...
            if sheet == "cm_in":
                if sig_b_conversion != 1:
                    node.append(
                        f'  @{sig_b_namespace}::{sig_b_name} = @CarMaker::{namespace}::{signal_name} * ({sig_b_conversion});')
                else:
...
def check_int_double_integrity(value_type, data, signal_name):
    """
    Checks if provided data is not empty

    Args:
      value_type (str): Value type of 'data' variable
      data (str) : Data to be checked for integrity
...
def save_file(file_path: str) -> None:
    """
    Save cm_mapping.cin file

    Args:
      file_path: Path to the cm_mapping.cin file
    Returns:
    """
...
def create_mapping_main_cm(workbook, file_path):
    ...
    Args:
      ...
      file_path: File path where cm_mapping.cin
