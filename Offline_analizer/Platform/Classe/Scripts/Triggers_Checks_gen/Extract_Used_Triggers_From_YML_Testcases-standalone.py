# -*- coding: utf-8 -*-
# @file Extract_Used_Triggers_From_YML_Testcases-standalone.py
...
def main_sequence():
    ...
    try:
        ...
        if f_list == []:
            ...
            #logger.logger.info(f"Total number of YML files processed                       = {len(f_list)}")
            #logger.info(f"Total number of used triggers found in YML files          = {len(list_found_triggers)}")

            Save_To_Excel(r'D:\ADAS_HIL\CustomerPrj\FDX\FDX_Database.xlsx')
            main_seq_result = 1
            return main_seq_result
    except Exception as e:
        main_seq_result = 0
