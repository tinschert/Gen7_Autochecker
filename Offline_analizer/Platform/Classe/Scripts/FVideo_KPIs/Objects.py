...
class Object:
    ...
    def cal_VX(self, df) -> dict:
        ...
        for idx, row in df.iterrows():
            ...
            if row["identifier"] == "vx_raw_gt":
                ...
                dict_dx_data["cam"] = self.getTraceDataSigBased(row["Message"]+"::"+row["Name"])
        if len(temp)!=2:
            logger.error("obj vx error occured when processing input database")
    ...
    def evaluate_VX(self,calculated_vx):
        """
        ...
        split_data= self.splitOnRangeByDx("vx", calculated_vx, deepcopy(self.evaluation_database["vx"]))
        ...
        for i in range(len(far_range_cam[0])):
            ...
            if gt_index > 0:
                gt_value = far_range_gt[1][gt_index]
                result = self.comapareDiff(gt_value,camera_value,self.evaluation_database["vx"]["far"]["diff"])
                evaluation_result["far"]["total_camera_gt_pairs"] += 1
                if result:
                    evaluation_result["far"]["passed_samples"] += 1
                else:
        ...
        for i in range(len(mid_range_cam[0])):
            ...
            if gt_index > 0:
                gt_value = mid_range_gt[1][gt_index]
                result = self.comapareDiff(gt_value,camera_value,self.evaluation_database["vx"]["mid"]["diff"])
                evaluation_result["mid"]["total_camera_gt_pairs"] += 1
                if result:
                    evaluation_result["mid"]["passed_samples"] += 1
                else:
        ...
        for i in range(len(near_range_cam[0])):
            ...
            if gt_index > 0:
                gt_value = near_range_gt[1][gt_index]
                result = self.comapareDiff(gt_value,camera_value,self.evaluation_database["vx"]["near"]["diff"])
                evaluation_result["near"]["total_camera_gt_pairs"] += 1
                if result:
                    evaluation_result["near"]["passed_samples"] += 1
                else:
        ...
        for key,value in temp_iter.items():
            temp = {}
            if (value["total_camera_gt_pairs"]):
                expected = self.evaluation_database["vx"][key]["required_verdict_per"]
                actual = round((value["passed_samples"] / value["total_camera_gt_pairs"])*100, 2)
                temp["expected"] = expected
                temp["actual"] = actual
                temp["verdict"] = "PASS" if actual>=expected else "FAIL"
            else:
                temp["expected"] = self.evaluation_database["vx"][key]["required_verdict_per"]
                temp["actual"] = "-"
                temp["verdict"] = "FAIL"
            evaluation_result[key]["result"] = temp
    ...
    def cal_VY(self, df) -> dict:
        ...
        for idx, row in df.iterrows():
            ...
            if row["identifier"] == "vy_raw_gt":
                ...
                dict_dx_data["cam"] = self.getTraceDataSigBased(row["Message"]+"::"+row["Name"])
        if len(temp)!=2:
            logger.error("obj Vy error occured when processing input database")
    ...
    def evaluate_VY(self,calculated_vy):
        """
        ...
        split_data= self.splitOnRangeByDx("vy", calculated_vy, deepcopy(self.evaluation_database["vy"]))
        ...
        for i in range(len(far_range_cam[0])):
            ...
            if gt_index > 0:
                gt_value = far_range_gt[1][gt_index]
                result = self.comapareDiff(gt_value,camera_value,self.evaluation_database["vy"]["far"]["diff"])
                evaluation_result["far"]["total_camera_gt_pairs"] += 1
                if result:
                    evaluation_result["far"]["passed_samples"] += 1
                else:
        ...
        for i in range(len(mid_range_cam[0])):
            ...
            if gt_index > 0:
                gt_value = mid_range_gt[1][gt_index]
                result = self.comapareDiff(gt_value,camera_value,self.evaluation_database["vy"]["mid"]["diff"])
                evaluation_result["mid"]["total_camera_gt_pairs"] += 1
                if result:
                    evaluation_result["mid"]["passed_samples"] += 1
                else:
        ...
        for i in range(len(near_range_cam[0])):
            ...
            if gt_index > 0:
                gt_value = near_range_gt[1][gt_index]
                result = self.comapareDiff(gt_value,camera_value,self.evaluation_database["vy"]["near"]["diff"])
                evaluation_result["near"]["total_camera_gt_pairs"] += 1
                if result:
                    evaluation_result["near"]["passed_samples"] += 1
                else:
        ...
        for key,value in temp_iter.items():
            temp = {}
            if (value["total_camera_gt_pairs"]):
                expected = self.evaluation_database["vy"][key]["required_verdict_per"]
                actual = round((value["passed_samples"] / value["total_camera_gt_pairs"])*100, 2)
                temp["expected"] = expected
                temp["actual"] = actual
                temp["verdict"] = "PASS" if actual>=expected else "FAIL"
            else:
                temp["expected"] = self.evaluation_database["vy"][key]["required_verdict_per"]
                temp["actual"] = "-"
                temp["verdict"] = "FAIL"
            evaluation_result[key]["result"] = temp
    ...
    def cal_AX(self, df) -> dict:
        ...
        for idx, row in df.iterrows():
            ...
            if row["identifier"] == "ax_raw_gt":
                ...
                dict_dx_data["cam"] = self.getTraceDataSigBased(row["Message"]+"::"+row["Name"])
        if len(temp)!=2:
            logger.error("obj Ax error occured when processing input database")
    ...
    def evaluate_AX(self,calculated_ax):
        """
        ...
        split_data= self.splitOnRangeByDx("ax", calculated_ax, deepcopy(self.evaluation_database["ax"]))
        ...
        for i in range(len(far_range_cam[0])):
            ...
            if gt_index > 0:
                gt_value = far_range_gt[1][gt_index]
                result = self.comapareDiff(gt_value,camera_value,self.evaluation_database["ax"]["far"]["diff"])
                evaluation_result["far"]["total_camera_gt_pairs"] += 1
                if result:
                    evaluation_result["far"]["passed_samples"] += 1
                else:
        ...
        for i in range(len(mid_range_cam[0])):
            ...
            if gt_index > 0:
                gt_value = mid_range_gt[1][gt_index]
                result = self.comapareDiff(gt_value,camera_value,self.evaluation_database["ax"]["mid"]["diff"])
                evaluation_result["mid"]["total_camera_gt_pairs"] += 1
                if result:
                    evaluation_result["mid"]["passed_samples"] += 1
                else:
        ...
        for i in range(len(near_range_cam[0])):
            ...
            if gt_index > 0:
                gt_value = near_range_gt[1][gt_index]
                result = self.comapareDiff(gt_value,camera_value,self.evaluation_database["ax"]["near"]["diff"])
                evaluation_result["near"]["total_camera_gt_pairs"] += 1
                if result:
                    evaluation_result["near"]["passed_samples"] += 1
                else:
        ...
        for key,value in temp_iter.items():
            temp = {}
            if (value["total_camera_gt_pairs"]):
                expected = self.evaluation_database["ax"][key]["required_verdict_per"]
                actual = round((value["passed_samples"] / value["total_camera_gt_pairs"])*100, 2)
                temp["expected"] = expected
                temp["actual"] = actual
                temp["verdict"] = "PASS" if actual>=expected else "FAIL"
            else:
                temp["expected"] = self.evaluation_database["ax"][key]["required_verdict_per"]
                temp["actual"] = "-"
                temp["verdict"] = "FAIL"
            evaluation_result[key]["result"] = temp
    ...
    def cal_AY(self, df) -> dict:
        ...
        for idx, row in df.iterrows():
            ...
            if row["identifier"] == "ay_raw_gt":
                ...
                dict_dx_data["cam"] = self.getTraceDataSigBased(row["Message"]+"::"+row["Name"])
        if len(temp)!=2:
            logger.error("obj Ay error occured when processing input database")
    ...
    def evaluate_AY(self,calculated_ay):
        """
        ...
        split_data= self.splitOnRangeByDx("ay", calculated_ay, deepcopy(self.evaluation_database["ay"]))
        ...
        for i in range(len(far_range_cam[0])):
            ...
            if gt_index > 0:
                gt_value = far_range_gt[1][gt_index]
                result = self.comapareDiff(gt_value,camera_value,self.evaluation_database["ay"]["far"]["diff"])
                evaluation_result["far"]["total_camera_gt_pairs"] += 1
                if result:
                    evaluation_result["far"]["passed_samples"] += 1
                else:
        ...
        for i in range(len(mid_range_cam[0])):
            ...
            if gt_index > 0:
                gt_value = mid_range_gt[1][gt_index]
                result = self.comapareDiff(gt_value,camera_value,self.evaluation_database["ay"]["mid"]["diff"])
                evaluation_result["mid"]["total_camera_gt_pairs"] += 1
                if result:
                    evaluation_result["mid"]["passed_samples"] += 1
                else:
        ...
        for i in range(len(near_range_cam[0])):
            ...
            if gt_index > 0:
                gt_value = near_range_gt[1][gt_index]
                result = self.comapareDiff(gt_value,camera_value,self.evaluation_database["ay"]["near"]["diff"])
                evaluation_result["near"]["total_camera_gt_pairs"] += 1
                if result:
                    evaluation_result["near"]["passed_samples"] += 1
                else:
        ...
        for key,value in temp_iter.items():
            temp = {}
            if (value["total_camera_gt_pairs"]):
                expected = self.evaluation_database["ay"][key]["required_verdict_per"]
                actual = round((value["passed_samples"] / value["total_camera_gt_pairs"])*100, 2)
                temp["expected"] = expected
                temp["actual"] = actual
                temp["verdict"] = "PASS" if actual>=expected else "FAIL"
            else:
                temp["expected"] = self.evaluation_database["ay"][key]["required_verdict_per"]
                temp["actual"] = "-"
                temp["verdict"] = "FAIL"
            evaluation_result[key]["result"] = temp
    ...
    def cal_length(self, df) -> dict:
        ...
        for idx, row in df.iterrows():
            #print(row["identifier"])
            if row["identifier"] == "length_raw_gt":
                temp["gt"] = row["Name"]+"_"+row["Message"].split("::")[-1]
                dict_dx_data["gt"] = self.getTraceDataSigBased(row["Message"]+"."+row["Name"])
                #dict_dx_data["gt"] = self.removeOutliers(dict_dx_data["gt"])
                #GT dx conversion, shifting, etc..
                ...
                dict_dx_data["cam"] = self.getTraceDataSigBased(row["Message"]+"::"+row["Name"])
        if len(temp)!=2:
            logger.error("obj length error occured when processing input database")
    ...
    def evaluate_length(self,calculated_length):
        """
        ...
        split_data = calculated_length

        #FAR -----------------------------------------------------------------------------
        gt = calculated_length["gt"]   #list [[timestamps], [smaples]]
        cam = calculated_length["cam"]
        ...
        for i in range(len(cam[0])):
            ...
            if gt_index > 0:
                ...
                result = self.comapareDiff(gt_value,camera_value,self.evaluation_database["length"]["diff"])
        ...
        if (evaluation_result["total_camera_gt_pairs"]):
            expected = self.evaluation_database["length"]["required_verdict_per"]
    ...
    def cal_width(self, df) -> dict:
        ...
        for idx, row in df.iterrows():
            #print(row["identifier"])
            if row["identifier"] == "width_raw_gt":
                temp["gt"] = row["Name"]+"_"+row["Message"].split("::")[-1]
                dict_dx_data["gt"] = self.getTraceDataSigBased(row["Message"]+"."+row["Name"])
                #dict_dx_data["gt"] = self.removeOutliers(dict_dx_data["gt"])
                #GT dx conversion, shifting, etc..
    ...
    def evaluate_width(self,calculated_width):
        """
        ...
        gt = calculated_width["gt"]   #list [[timestamps], [smaples]]
        cam = calculated_width["cam"]
        ...
        for i in range(len(cam[0])):
            ...
            if gt_index > 0:
                gt_value = gt[1][gt_index]
                result = self.comapareDiff(gt_value,camera_value,self.evaluation_database["width"]["diff"])
                evaluation_result["total_camera_gt_pairs"] += 1
                if result:
                    evaluation_result["passed_samples"] += 1
                else:
        ...
        temp = {}
        if (evaluation_result["total_camera_gt_pairs"]):
            expected = self.evaluation_database["width"]["required_verdict_per"]
            actual = (evaluation_result["passed_samples"] / evaluation_result["total_camera_gt_pairs"])*100
            temp["expected"] = expected
            temp["actual"] = actual
            temp["verdict"] = "PASS" if actual>=expected else "FAIL"
        else:
            temp["expected"] = self.evaluation_database["width"]["required_verdict_per"]
            temp["actual"] = "-"
            temp["verdict"] = "FAIL"
        evaluation_result["result"] = temp
    ...
    def cal_headingangle(self, df) -> dict:
        """
        this will get signal data from file and
        then apply calculations if needed for data like timeshift, fmu
        ...
        logger.info(f" ---headingangle---")
        temp={} #to store keys to identify which is gt and which is camera sig
        dict_dx_data = {}
        for idx, row in df.iterrows():
            #print(row["identifier"])
            if row["identifier"] == "headingangle_raw_gt":
                temp["gt"] = row["Name"]+"_"+row["Message"].split("::")[-1]
                dict_dx_data["gt"] = self.getTraceDataSigBased(row["Message"]+"."+row["Name"])
                #dict_dx_data["gt"] = self.removeOutliers(dict_dx_data["gt"])
                #GT dx conversion, shifting, etc..
                ...
                dict_dx_data["cam"] = self.getTraceDataSigBased(row["Message"]+"::"+row["Name"])
        if len(temp)!=2:
            logger.error("obj heading angle error occured when processing input database")
    ...
    def evaluate_headingangle(self,calculated_headingangle):
        ...
        #split_data = self.splitOnRangeByDx("headingangle", calculated_headingangle, deepcopy(self.evaluation_database["headingangle"]))
        split_data = calculated_headingangle
    ...
    def evaluate_type(self,calculated_type):
        """
        ...
        for i in range(len(cam[0])):
            ...
            result = self.compareValue(camera_value,self.evaluation_database["type"]["value"])
            if result:
                evaluation_result["passed_samples"] += 1
            else:
                evaluation_result["failed_samples"] += 1
        ...
        if (evaluation_result["total_camera_samples"]):
            expected = self.evaluation_database["type"]["required_verdict_per"]
    ...
    def cal_probexist(self, df) -> dict:
        """
        this will get signal data from file and
        then apply calculations if needed for data like timeshift, fmu
        ...
        logger.info(f" ---probexist---")
        temp={} #to store keys to identify which is gt and which is camera sig
        dict_dx_data = {}
        for idx, row in df.iterrows():
            #print(row["identifier"])
            if row["identifier"] == "probexist_raw_gt":
                temp["gt"] = row["Name"]+"_"+row["Message"].split("::")[-1]
                dict_dx_data["gt"] = self.getTraceDataSigBased(row["Message"]+"."+row["Name"])
                #dict_dx_data["gt"] = self.removeOutliers(dict_dx_data["gt"])
                #GT dx conversion, shifting, etc..
                ...
                dict_dx_data["cam"] = self.getTraceDataSigBased(row["Message"]+"::"+row["Name"])
        if len(temp)!=1:
            logger.error("obj probexist error occured when processing input database")
    ...
    def evaluate_probexist(self,calculated_probexist):
        """
        ...
        for i in range(len(cam[0])):
            ...
            result = self.compareValue(camera_value,self.evaluation_database["probexist"]["value"])
            if result:
                evaluation_result["passed_samples"] += 1
            else:
                evaluation_result["failed_samples"] += 1
        ...
        temp = {}
        if (evaluation_result["total_camera_samples"]):
            expected = self.evaluation_database["probexist"]["required_verdict_per"]
            actual = (evaluation_result["passed_samples"] / evaluation_result["total_camera_samples"])*100
            temp["expected"] = expected
            temp["actual"] = actual
            temp["verdict"] = "PASS" if actual>=expected else "FAIL"
        else:
            temp["expected"] = self.evaluation_database["probexist"]["required_verdict_per"]
            temp["actual"] = "-"
            temp["verdict"] = "FAIL"
        evaluation_result["result"] = temp
    ...
    def cal_scalechange(self, df) -> dict:
        """
        this will get signal data from file and
        then apply calculations if needed for data like timeshift, fmu
        ...
        logger.info(f" ---scalechange---")
        temp={} #to store keys to identify which is gt and which is camera sig
        dict_dx_data = {}
        for idx, row in df.iterrows():
            #print(row["identifier"])
            if row["identifier"] == "scalechange_raw_gt":
                temp["gt"] = row["Name"]+"_"+row["Message"].split("::")[-1]
                dict_dx_data["gt"] = self.getTraceDataSigBased(row["Message"]+"."+row["Name"])
                #dict_dx_data["gt"] = self.removeOutliers(dict_dx_data["gt"])
                #GT dx conversion, shifting, etc..
                ...
                dict_dx_data["cam"] = self.getTraceDataSigBased(row["Message"]+"::"+row["Name"])
        if len(temp)!=1:
            logger.error("obj scale change error occured when processing input database")
    ...
    def evaluate_scalechange(self,calculated_scalechange):
        """
        

        Args:
          calculated_scalechange: 
        ...
        #split_data = self.splitOnRangeByDx("scalechange", calculated_scalechange, deepcopy(self.evaluation_database["scalechange"]))
        split_data = calculated_scalechange

        scalechange_cam = calculated_scalechange["cam"]
        
        evaluation_result["total_camera_samples"] = len(scalechange_cam[0])
        evaluation_result["camera_missing_samples_count"] = self.validateTimestamp(scalechange_cam[0], self.camera_tolerance_time)
        

        for i in range(1, len(self.pair_indices)):
            current_indices = self.pair_indices[i]
            previous_indices = self.pair_indices[i-1]

            #gt
            current_gt_value = self.calculated_parameters_dict["dx"]["gt"][1][current_indices["gt_index"]]
            previous_gt_value = self.calculated_parameters_dict["dx"]["gt"][1][previous_indices["gt_index"]]
            #camera
            current_camera_value = self.calculated_parameters_dict["dx"]["cam"][1][current_indices["cam_index"]]
            previous_camera_value = self.calculated_parameters_dict["dx"]["cam"][1][previous_indices["cam_index"]]

            if (current_gt_value > previous_gt_value): #SC < 1
                if (current_camera_value > previous_camera_value) and (scalechange_cam[1][current_indices["cam_index"]] < 1):
                    evaluation_result["passed_samples"]+=1  #PASS
                else:
                    evaluation_result["failed_samples"]+=1   #FAIL
            
            elif (current_gt_value < previous_gt_value):  #SC > 1
                if (current_camera_value < previous_camera_value) and (scalechange_cam[1][current_indices["cam_index"]] > 1):
                    evaluation_result["passed_samples"]+=1  #PASS
                else:
                    evaluation_result["failed_samples"]+=1   #FAIL
                ...
                if (current_camera_value == previous_camera_value) and (float(scalechange_cam[1][current_indices["cam_index"]]) == float(1)):
                    evaluation_result["passed_samples"]+=1  #PASS
                else:
                    evaluation_result["failed_samples"]+=1   #FAIL
        ...
        temp = {}
        if (evaluation_result["total_camera_samples"]):
            expected = self.evaluation_database["scalechange"]["required_verdict_per"]
            actual = (evaluation_result["passed_samples"] / (evaluation_result["total_camera_samples"]-1))*100
            temp["expected"] = expected
            temp["actual"] = actual
            temp["verdict"] = "PASS" if actual>=expected else "FAIL"
        else:
            temp["expected"] = self.evaluation_database["scalechange"]["required_verdict_per"]
            temp["actual"] = "-"
            temp["verdict"] = "FAIL"
        evaluation_result["result"] = temp
    ...
    def cal_typeprob(self, df) -> dict:
        ...
        logger.info(f" ---typeprob---")
        temp={} #to store keys to identify which is gt and which is camera sig
        dict_dx_data = {}
        for idx, row in df.iterrows():
            #print(row["identifier"])
            if row["identifier"] == "typeprob_raw_gt":
                temp["gt"] = row["Name"]+"_"+row["Message"].split("::")[-1]
                dict_dx_data["gt"] = self.getTraceDataSigBased(row["Message"]+"."+row["Name"])
                #dict_dx_data["gt"] = self.removeOutliers(dict_dx_data["gt"])
                #GT dx conversion, shifting, etc..
                ...
                dict_dx_data["cam"] = self.getTraceDataSigBased(row["Message"]+"::"+row["Name"])
        if len(temp)!=1:
            logger.error("obj type prob occured when processing input database")
    ...
    def evaluate_typeprob(self,calculated_typeprob):
        """
        ...
        #split_data = self.splitOnRangeByDx("typeprob", calculated_typeprob, deepcopy(self.evaluation_database["typeprob"]))
        split_data = calculated_typeprob

        cam = calculated_typeprob["cam"]
        ...
        for i in range(len(cam[0])):
            ...
            result = self.compareValue(camera_value,self.evaluation_database["typeprob"]["value"])
            if result:
                evaluation_result["passed_samples"] += 1
            else:
                evaluation_result["failed_samples"] += 1
        ...
        temp = {}
        if (evaluation_result["total_camera_samples"]):
            expected = self.evaluation_database["typeprob"]["required_verdict_per"]
            actual = (evaluation_result["passed_samples"] / evaluation_result["total_camera_samples"])*100
            temp["expected"] = expected
            temp["actual"] = actual
            temp["verdict"] = "PASS" if actual>=expected else "FAIL"
        else:
            temp["expected"] = self.evaluation_database["typeprob"]["required_verdict_per"]
            temp["actual"] = "-"
            temp["verdict"] = "FAIL"
        evaluation_result["result"] = temp
    ...
    def cal_visibleview(self, df) -> dict:
        """
        this will get signal data from file and
        then apply calculations if needed for data like timeshift, fmu
        ...
        logger.info(f" ---visibleview---")
        temp={} #to store keys to identify which is gt and which is camera sig
        dict_dx_data = {}
        for idx, row in df.iterrows():
            #print(row["identifier"])
            if row["identifier"] == "visibleview_raw_gt":
                temp["gt"] = row["Name"]+"_"+row["Message"].split("::")[-1]
                dict_dx_data["gt"] = self.getTraceDataSigBased(row["Message"]+"."+row["Name"])
                #dict_dx_data["gt"] = self.removeOutliers(dict_dx_data["gt"])
                #GT dx conversion, shifting, etc..
    ...
    def evaluate_visibleview(self,calculated_visibleview):
        """
        

        Args:
          calculated_visibleview: 
        ...
        #split_data = self.splitOnRangeByDx("visibleview", calculated_visibleview, deepcopy(self.evaluation_database["visibleview"]))
        split_data = calculated_visibleview

        cam = calculated_visibleview["cam"]
        ...
        for i in range(len(cam[0])):
            ...
            result = self.compareValue(camera_value,self.evaluation_database["visibleview"]["value"])
            if result:
                evaluation_result["passed_samples"] += 1
            else:
                evaluation_result["failed_samples"] += 1
        ...
        temp = {}
        if (evaluation_result["total_camera_samples"]):
            expected = self.evaluation_database["visibleview"]["required_verdict_per"]
            actual = (evaluation_result["passed_samples"] / evaluation_result["total_camera_samples"])*100
            temp["expected"] = expected
            temp["actual"] = actual
            temp["verdict"] = "PASS" if actual>=expected else "FAIL"
        else:
            temp["expected"] = self.evaluation_database["visibleview"]["required_verdict_per"]
            temp["actual"] = "-"
            temp["verdict"] = "FAIL"
        evaluation_result["result"] = temp
