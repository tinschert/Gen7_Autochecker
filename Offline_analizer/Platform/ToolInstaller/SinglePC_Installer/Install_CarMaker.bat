...
:: Version 12.0.2
:: 2023.11.14
:: Change: Installation updated to CarMaker 12.0.2 and CANoe AddOn for CANoe 16 and 17.
...
set pad1="C:\Program Files\Vector CANoe 16\Installer Additional Components\CarMaker\VectorCANoeCarMakerAddOn_2.8.1.exe"
set pad2="C:\Program Files\Vector CANoe 17\Installer Additional Components\CarMaker\VectorCANoeCarMakerAddOn_2.8.2.exe"
...
:Install_CANoe_AddOn
IF EXIST %pad1% (%pad1% /S /v"/qn INSTALLDIR=\"c:\Program Files\Vector CANoe 16\"") ELSE (ECHO "Please verify the installation of Vector CANoe CarMaker AddOn for CANoe16")
IF %ERRORLEVEL% EQU 0 (ECHO Vector CarMaker AddOn : OK>> output.log) ELSE (ECHO Vector CarMaker AddOn CANoe 16  : To be checked>> output.log)
IF EXIST %pad2% (%pad2% /S /v"/qn INSTALLDIR=\"c:\Program Files\Vector CANoe 17\"") ELSE (ECHO "Please verify the installation of Vector CANoe CarMaker AddOn for CANoe 17")
IF %ERRORLEVEL% EQU 0 (ECHO Vector CarMaker AddOn : OK>> output.log) ELSE (ECHO Vector CarMaker AddOn CANoe17 : To be checked>> output.log)
