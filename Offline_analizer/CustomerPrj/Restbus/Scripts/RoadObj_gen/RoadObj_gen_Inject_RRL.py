# -*- coding: utf-8 -*-
# @file RoadObj_gen_Inject_RRL.py
...
/**
 * @file Inject_Radar_RL.cin
 * @author ADAS_HIL_TEAM
 * @date 03-21-2023
 * @brief Handles injection of rear left radar
 * 
 * ##################################################################
 * C O P Y R I G H T S
 * ----------------------------------------------------------------
...
variables
{
  int target_counter_radar_rl = 0;
  int injection_counter_radar_rl = -1;
}
'''

def update_Inject_RRL(file_path, num_objects):
    """
    Description: Main function for updating the injection file for the Radar Rear Left which orchestrates the update functions and saves the data in the file.
    ...
    update_inject_radar_rl_hdr()
    update_inject_radar_rl_obj(num_objects)
    
    update_delete_radar_rl_bus(num_objects)
    update_delete_radar_rl_xcp(num_objects)
...
def update_inject_radar_rl_hdr():
    """ 
    ...
    script_list.append('  */')
    script_list.append('void inject_radar_rl_hdr(){')
    script_list.append('  @CAN_RadarRL::RRL_Object::RRL_Object_ObjHdr_NumValidObjects = @target_radar_rl_sim::objdata.number_of_obj;')
    script_list.append('  @CAN_RadarRL::RRL_Object::RRL_Object_SpiHdr_SensorOriginPointX = @radarrl_par::mounting_pos_x;')
    script_list.append('  @CAN_RadarRL::RRL_Object::RRL_Object_SpiHdr_SensorOriginPointY = @radarrl_par::mounting_pos_y;')
    script_list.append('  @CAN_RadarRL::RRL_Object::RRL_Object_SpiHdr_SensorOriginPointZ = @radarrl_par::mounting_pos_z;')
    script_list.append('  @CAN_RadarRL::RRL_Object::RRL_Object_SpiHdr_SensorOrientYaw = @radarrl_par::mounting_ori_yaw;')
    script_list.append('    ')
    script_list.append('    // Set the number of objects to be put into the CAN TP ByteArray to be the object count, but atleast 1')
    script_list.append('    if (@target_radar_rl_sim::objdata.number_of_obj >= 1) @CAN_RadarRL_RRL_Object::Write_No_signals = @target_radar_rl_sim::objdata.number_of_obj; else @CAN_RadarRL_RRL_Object::Write_No_signals = 1;')
    script_list.append('}')
    script_list.append('')

def update_inject_radar_rl_obj(num_objects):
    """
    ...
    script_list.append('/** @brief Injects calculated object data stored in the target_radar_rl_sim arrays.')
    ...
    script_list.append('  * @return void')
    script_list.append('  */')
    script_list.append('void inject_radar_rl_obj(byte object_id, int injection_method)')
    script_list.append('{')
    script_list.append('  target_counter_radar_rl = 0;')
    script_list.append('  injection_counter_radar_rl = -1;')
    script_list.append('  ')
    script_list.append('  for(target_counter_radar_rl = 0; target_counter_radar_rl <= object_id; target_counter_radar_rl++){')
    script_list.append('    if(@target_radar_rl_sim::objdata.empty_obj[target_counter_radar_rl] == 0 && @target_radar_rl_sim::objdata.empty_obj[object_id] == 0){')
    ...
    script_list.append('      injection_counter_radar_rl++;')
    script_list.append('    }')
    script_list.append('  }')
    script_list.append('  ')
    ...
    script_list.append('    switch (injection_counter_radar_rl)')
    script_list.append('    {')
    
    for obj_index in range(num_objects):
        obj_id = obj_index+1
        prefix = '@CAN_RadarRL::RRL_Object::RRL_Object_Obj{0}'.format("0"+str(obj_id) if obj_id < 10 else str(obj_id))
        object_buffer = '@CAN_RadarRL::RRL_Object::RRL_Object_Obj{0}'.format(str(obj_id) if obj_id < 10 else str(obj_id))
        ...
        script_list.append('          */')
        script_list.append(f'        {object_buffer}.ObjId = injection_counter_radar_rl + 1;')
        ...
        script_list.append('          */')
        script_list.append(f'        {object_buffer}.OrientationYaw = dllGetRadarRLObjYawAngle(@hil_ctrl::Handle_RoadObj, object_id);')
        script_list.append(f'        {object_buffer}.Width = dllGetRadarRLInputObjectWidth(@hil_ctrl::Handle_RoadObj, object_id);')
        script_list.append(f'        {object_buffer}.Length = dllGetRadarRLInputObjectLength(@hil_ctrl::Handle_RoadObj, object_id);')
        script_list.append(f'        {object_buffer}.RCS = dllGetRadarRLObjRa6RCS(@hil_ctrl::Handle_RoadObj, object_id);')
        script_list.append(f'        {object_buffer}.Px = dllGetRadarRLObjDistX(@hil_ctrl::Handle_RoadObj, object_id);')
        script_list.append(f'        {object_buffer}.Py = dllGetRadarRLObjDistY(@hil_ctrl::Handle_RoadObj, object_id);')
        script_list.append(f'        {object_buffer}.Vx = dllGetRadarRLObjVelX(@hil_ctrl::Handle_RoadObj, object_id);')
        script_list.append(f'        {object_buffer}.Vy = dllGetRadarRLObjVelY(@hil_ctrl::Handle_RoadObj, object_id);')
        script_list.append(f'        {object_buffer}.Ax = dllGetRadarRLObjAccelX(@hil_ctrl::Handle_RoadObj, object_id);')
        script_list.append(f'        {object_buffer}.Ay = dllGetRadarRLObjAccelY(@hil_ctrl::Handle_RoadObj, object_id);')
        script_list.append(f'        {object_buffer}.MovingStatus = dllGetRadarRLObjRa6MovingStatus(@hil_ctrl::Handle_RoadObj, object_id);')
        script_list.append(f'        {object_buffer}.Age += front_corner_cycle_time * 10; //50 ms * 10 = unit is 100us')
        script_list.append(f'        {object_buffer}.ProbClassType = dllGetRadarRLObjRa6ObjType(@hil_ctrl::Handle_RoadObj, object_id);')
        script_list.append(f'        {object_buffer}.ReferencePoint = dllGetRadarRLObjRefPnt(@hil_ctrl::Handle_RoadObj, object_id);')
    ...
    print('Inject Radar_RL updated successfully.')

def update_delete_radar_rl_bus(num_objects):
    """
    ...
    script_list.append('  * @return void')
    script_list.append('  */')
    script_list.append('void delete_radar_rl_bus(byte object_id)')
    script_list.append('{')
    script_list.append('  target_counter_radar_rl = 0;')
    script_list.append('  injection_counter_radar_rl = -1;')
    script_list.append('  ')
    script_list.append('  for(target_counter_radar_rl = 0; target_counter_radar_rl <= object_id; target_counter_radar_rl++){')
    script_list.append('    if(@target_radar_rl_sim::objdata.empty_obj[target_counter_radar_rl] == 0 || target_counter_radar_rl == object_id){')
    script_list.append('      injection_counter_radar_rl++;')
    script_list.append('    }')
    script_list.append('  }')
    script_list.append('  ')
    ...
    print('Delete Radar_RL updated successfully.')

def update_delete_radar_rl_xcp(num_objects):
    """
    ...
    script_list.append('  * @return void')
    script_list.append('  */')
    script_list.append('void delete_radar_rl_xcp(byte object_id)')
    script_list.append('{')
    script_list.append('  /**')
    script_list.append('    * TBD')
    script_list.append('    */')
