...
#include <DataDict.h> //IPG-SolEng_For_UAQ_Access_to_Infofile
#include <InfoUtils.h> //IPG-SolEng_For_UAQ_Access_to_Infofile
#include <SimCore.h> //IPG-SolEng_For_UAQ_Access_to_Infofile
double VehMass = 0; //IPG-SolEng_For_UAQ_Access_to_Infofile
double VehOuterSkin[6];
double *VehOuterSkin_Inf;
double OuterSkin_Def[6] ={0,0,0,0,0,0};
double TrafficDimensions[3];
double *TrafficDimensions_Inf;
double TDimensions_Def[3] ={0,0,0}; 

#include "IOVec.h"
#include "User.h"

/* @@PLUGIN-BEGIN-INCLUDE@@ - Automatically generated code - don't edit! */
#include "Bosch_VhclCtrl_AEB_CarMaker_rtw/Bosch_VhclCtrl_AEB_wrap.h"
#include "BoschAEB_Hyd_CarMaker_rtw/BoschAEB_Hyd_wrap.h"
/* @@PLUGIN-END@@ */
...
User_Register (void)
{
    ...
    Bosch_VhclCtrl_AEB_Register();
    BoschAEB_Hyd_Register();
    /* @@PLUGIN-END@@ */
...
User_DeclQuants (void)
{
	...
	DDefDouble4(NULL, "EgoVehicle.Mass", "Kg", &VehMass, DVA_None); //IPG-SolEng_For_UAQ_Access_to_Infofile
	
	char sbuf[32];
	sprintf (sbuf, "EgoVehicle.Length");
	DDefDouble(NULL,sbuf, "",VehOuterSkin+3, DVA_None);
	
	sprintf (sbuf, "EgoVehicle.Width1");
	DDefDouble(NULL,sbuf, "",VehOuterSkin+1 , DVA_None);
	
	sprintf (sbuf, "EgoVehicle.Width2");
	DDefDouble(NULL,sbuf, "",VehOuterSkin+4 , DVA_None);
	 	
	sprintf (sbuf, "Traffic.0.Length");
	DDefDouble(NULL,sbuf, "",TrafficDimensions+0 , DVA_None);
	
	sprintf (sbuf, "Traffic.0.Width");
	DDefDouble(NULL,sbuf, "",TrafficDimensions+1 , DVA_None);
...
{
	if (SimCore.State == SCState_StartWait)
	{
		VehMass = iGetDblOpt(SimCore.Vhcl.Inf, "Body.mass", 1000); //IPG-SolEng_For_UAQ_Access_to_Infofile
		
		VehOuterSkin_Inf = iGetFixedTableOpt2(SimCore.Vhcl.Inf, "Vehicle.OuterSkin", OuterSkin_Def,6, 1);
		memcpy(VehOuterSkin, VehOuterSkin_Inf, 6*sizeof(double));
		
		TrafficDimensions_Inf = iGetFixedTableOpt2(SimCore.TestRun.Inf, "Traffic.0.Basics.Dimension", TDimensions_Def,3, 1);
		memcpy(TrafficDimensions, TrafficDimensions_Inf, 3*sizeof(double));
